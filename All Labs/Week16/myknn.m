    ...
    methods(Static)
        ...
        function [predictions, scores] = predict(m, test_examples)
            ...
            for i=1:size(test_examples,1)
                ...
                [this_prediction, this_score] = myknn.predict_one(m, this_test_example);
        ...
        function [prediction, scores] = predict_one(m, this_test_example)
            ...
            [prediction, scores] = myknn.make_prediction(m, neighbour_indices);
        ...
        function [prediction, scores] = make_prediction(m, neighbour_indices)
            
			neighbour_labels = m.train_labels(neighbour_indices);
            
            scoresArray = [];
            uniqueLabels = unique(m.train_labels);
            
            for i = 1:size(uniqueLabels)
                scoresArray(end+1) = sum(neighbour_labels == uniqueLabels(i));
            end
