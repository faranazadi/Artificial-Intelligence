    ...
    methods(Static)
        ...
        function [predictions, scores] = predict(m, test_examples)
            ...
            for i=1:size(test_examples,1)
                ...
                [this_prediction, this_score] = mytree.predict_one(m, this_test_example);
                predictions(end+1) = this_prediction;
                
                scores = cat(1,scores,this_score);
            
			end
        end

        function [prediction, scores] = predict_one(m, this_test_example)
            
			node = mytree.descend_tree(m.tree, this_test_example);
            prediction = node.prediction;
            % create an array to hold the scores of each label
            scoresArray = [];
            % get the each of the unique labels:
            uniqueLabels =  m.unique_classes;
            % get the labels inside leaf node:
            nodeLabels = node.labels;
            
            % for each unique label:
            for i = 1:size(uniqueLabels)
                % add the number of node labels that correlate to the
                % unique label:
                numLabels = sum(nodeLabels == uniqueLabels(i));
                % divide the node labels by the total number of labels in
                % the node:
                scoresArray(end+1) = numLabels / length(nodeLabels);
            end
